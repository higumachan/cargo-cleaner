...
/// A [`Cell`] contains the [`Text`] to be displayed in a [`Row`] of a [`MultiHighlightTable`].
///
...
/// Holds data to be displayed in a [`MultiHighlightTable`] widget.
///
...
impl<'a> Row<'a> {
    ...
    pub fn new<T>(id: RowId, cells: T) -> Self
    where
        T: IntoIterator,
        T::Item: Into<Cell<'a>>,
    {
...
/// ```
#[derive(Debug, Default, Clone, Eq, PartialEq, Hash)]
pub struct MultiHighlightTable<'a> {
    ...
    rows: Vec<Row<'a>>,
    ///
    highlight: EnumMap<InnerRowHighlight, Highlight<'a>>,
    highlight_spacing: HighlightSpacing,
}

impl<'a> MultiHighlightTable<'a> {
    /// Creates a new [`MultiHighlightTable`] widget with the given rows.
    ///
    ...
    /// ```rust
    /// # use ratatui::{prelude::*, widgets::*};
    /// let table = MultiHighlightTable::new(vec![
    ///     Row::new(vec![
    ///         Cell::from("Cell1"),
    ///         Cell::from("Cell2")
    ///     ]),
    ...
    /// Creates a custom block around a [`MultiHighlightTable`] widget.
    ///
    /// The `block` parameter is of type [`Block`]. This holds the specified block to be
    /// created around the [`MultiHighlightTable`]
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    pub fn selected_highlight(mut self, highlight: Highlight<'a>) -> Self {
        self.highlight[InnerRowHighlight::Cursor] = highlight;
        self
    }

    pub fn add_highlight(mut self, key: RowHighlight, highlight: Highlight<'a>) -> Self {
        self.highlight[key.into()] = highlight;
        self
    }

    /// Creates a header for a [`MultiHighlightTable`] widget.
    ///
    /// The `header` parameter is of type [`Row`] and this holds the cells to be displayed at the
    /// top of the [`MultiHighlightTable`]
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    fn max_highlight_space(&self) -> u16 {
        self.highlight
            .iter()
            .map(|(_, h)| h.symbol.map_or(0, |s| s.width() as u16))
            .max()
            .unwrap_or(0)
    }
}

impl<'a> Styled for MultiHighlightTable<'a> {
    type Item = MultiHighlightTable<'a>;
...
#[derive(Debug, Clone, Eq, PartialEq, Hash, State, Enum)]
enum InnerRowHighlight {
    Cursor,
    User(RowHighlight),
}

impl From<RowHighlight> for InnerRowHighlight {
    fn from(r: RowHighlight) -> Self {
        InnerRowHighlight::User(r)
    }
}

#[derive(Debug, Clone, Copy, Eq, PartialEq, Hash, State, Enum)]
#[repr(u64)]
...
pub struct TableState {
    ...
    row_state: HashMap<RowId, StateSet<RowHighlight>>,
}

impl TableState {
    fn get_row_highlight(&self, row: &Row) -> Option<InnerRowHighlight> {
        if let Some(selected) = self.selected {
            if selected == row.id {
                return Some(InnerRowHighlight::Cursor);
            }
        }
        self.row_state.get(&row.id).and_then(|s| {
            s.into_iter()
                .min_by_key(|t| *t as u64)
                .map(|t| InnerRowHighlight::User(t))
        })
    }
    ...
    pub fn with_selected(mut self, selected: Option<RowId>) -> Self {
    ...
    pub fn select(&mut self, index: Option<RowId>) {
        self.selected = index;
        if index.is_none() {
            self.offset = 0;
        }
    ...
    pub fn add_row_state(&mut self, row_id: RowId, state: RowHighlight) {
        self.row_state
            .entry(row_id)
    ...
    pub fn remove_row_state(&mut self, row_id: RowId, state: RowHighlight) {
        self.row_state
            .entry(row_id)
            .or_insert_with(StateSet::new)
            .remove(state);
...
impl<'a> StatefulWidget for MultiHighlightTable<'a> {
    ...
    fn render(mut self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let selected_index = state
            .selected
            .and_then(|s| self.rows.iter().position(|r| r.id == s));
        let (start, end) = self.get_row_bounds(selected_index, state.offset, rows_height);
        state.offset = start;
            ...
            .take(end - start)
        {
            ...
            let highlight = state
                .get_row_highlight(table_row)
                .map(|t| &self.highlight[t]);
            let is_selected = state.selected.map_or(false, |s| s == table_row.id);
            if selection_width > 0 && is_selected {
                ...
                buf.set_stringn(
                    inner_offset,
                    row,
                    highlight.and_then(|h| h.symbol).unwrap_or(""),
            ...
            if let Some(highlight) = highlight {
                buf.set_style(table_row_area, highlight.style);
            }
        }
    }
}
...
impl<'a> Widget for MultiHighlightTable<'a> {
...
mod tests {
    ...
    fn table_invalid_percentages() {
        MultiHighlightTable::new(vec![]).widths(&[Constraint::Percentage(110)]);
    }
    ...
    mod table_column_widths {
        ...
        fn test(
            ...
            let table = MultiHighlightTable::new(vec![]).widths(constraints);
    ...
    fn table_can_be_stylized() {
        assert_eq!(
            MultiHighlightTable::new(vec![Row::new(vec![Cell::from("")])])
                .black()
                .on_white()
                .bold()
                .not_crossed_out()
